...
export default function PlatformExpansionJourney({ className }: { className?: string }) {
  ...
  return (
    <div className={cn('w-full', className)} data-platform-fade>
      ...
      <div className="relative hidden sm:block px-1 lg:px-8">
        <div
          aria-hidden
          className="absolute left-[12.5%] right-[8%] top-[42px] h-[2px] rounded-full"
          style={{
            background:
        ...
        <span
          aria-hidden
          className="absolute right-[8%] top-[42px] z-10 -translate-y-1/2 translate-x-full border-y-[5px] border-y-transparent border-l-[9px] border-l-[#F97316]"
        ...
        <div className="grid grid-cols-4 gap-2">
          {nodes.map((node) => {
            ...
            return (
              <div key={node.clinic} className="relative flex flex-col items-center text-center">
                <p className="mb-3 min-h-[32px] text-xs font-semibold leading-snug text-[#F8FAFC]">
                  {node.clinic}
                </p>
                <span
                  className={cn(
                    'relative z-10 mb-3 flex h-[14px] w-[14px] items-center justify-center rounded-full ring-[5px] ring-[#030712]',
